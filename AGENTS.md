...
This repo contains the HTD Org Whiteboard, a small React/Vite planning app for headcount and reporting-line planning. It is deployed on Render and uses a tiny Express API to persist one shared board state in GitHub.
...
- `data/board.json` is the shared saved board state. It is written by the API through the GitHub Contents API.
...
- App load: fetches `/api/board` from the shared API.
- Save button: sends the full board to `PUT /api/board`.
- API writes `data/board.json` back to GitHub.
- When a browser has no unsaved local edits, it polls every 6 seconds and applies newer shared board state.
...
- Saving the board creates commits to `data/board.json`, so local pushes may need `git pull --rebase` first.
